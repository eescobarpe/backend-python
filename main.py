...
app = FastAPI(
    ...
    description="Sistema de auditoría dinámico completo para arquitectura SilverNonStop - Conversión directa de automatizaciones Airtable",
    version="1.0.0"
)

# Modelos para el sistema de auditoría (basados en tu documentación y script)
class EventoAuditoriaBase(BaseModel):
    ...
    automatizacion_origen: Optional[str] = "Railway_API"
...
class DiagnosticoResponse(BaseModel):
    ...
    criticos_pendientes: int
    tablas_monitoreadas: List[str]
    metricas_silvernostop: Dict
    ultima_actualizacion: datetime

# Script SQL completo para crear todas las tablas (basado en tu documentación)
CREATE_COMPLETE_AUDIT_SYSTEM_SQL = """
-- Tabla principal de auditoría SilverNonStop (conversión directa de SilverNonStop_AuditLog)
...
-- Tabla de configuración dinámica (basada en ConfiguracionDinamicaAuditoria)
...
async def root():
    return {
        ...
        "version": "1.0.0",
        "database": "PostgreSQL",
        "arquitectura": "Sistema completo de auditoría desacoplado de Airtable",
        "conversion": "Conversión directa de automatizaciones JavaScript a API REST",
        "componentes": [
            ...
            "Sistema de duplicados con hash",
        ...
        "endpoints_principales": [
            ...
            "/configuracion-dinamica - Configuración actual"
        ]
    }
...
async def setup_sistema_completo():
    ...
    try:
        ...
        return {
            ...
            "conversion_airtable": {
                "tabla_origen": "SilverNonStop_AuditLog (Airtable)",
                "tabla_destino": "silvernostop_audit_log (PostgreSQL)",
                "funciones_convertidas": [
                    "crearLogSilverNonStop → procesar_evento_auditoria",
                    "logErrorCritico → /log-error-critico",
                    "logCampoFaltante → /log-campo-faltante",
                    "logErrorFEM → /log-error-fem",
                    "logErrorDespivotado → /log-error-despivotado",
                    "logInfo → /log-info"
                ]
            }
        }
...
# ENDPOINTS ESPECIALIZADOS (Conversión directa de tu script automatizacion_logging_silvernostop.js)
...
async def procesar_evento_auditoria(evento: EventoAuditoriaBase):
    """Función principal de procesamiento (conversión directa de tu script)"""
    try:
        conn = await get_db_connection()
        
        # Validar tabla origen (conversión de validarTablaOrigen)
        ...
        # Verificar si es duplicado (conversión de verificarDuplicadoRapido)
        ...
        if existing:
            # Incrementar contador de duplicado (conversión de incrementarContadorRapido)
            await conn.execute(
            ...
            # Crear nuevo evento (conversión de prepararLogEntry)
            ...
            # Si es crítico, log especial (conversión de alerta en script)
            ...
            return {
                ...
                "conversion_info": "Convertido desde automatización Airtable"
            }
            
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error procesando evento: {str(e)}")

@app.get("/diagnostico-completo", response_model=DiagnosticoResponse)
async def diagnostico_completo():
    ...
    try:
        ...
        # Métricas SilverNonStop (simuladas, en producción vendrían de Airtable)
        metricas_silvernostop = {
            "fems_minimos_configurados": 5,  # Sector, Rol, Equipo, Áreas, Países
            "nodos_mapa_valor_activos": 4,
        ...
        return DiagnosticoResponse(
            ...
            metricas_silvernostop=metricas_silvernostop,
...
async def obtener_metricas_silvernostop():
    ...
    try:
        ...
        return {
            ...
            "conversion_info": "Métricas convertidas desde sistema Airtable"
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error obteniendo métricas: {str(e)}")

@app.get("/configuracion-dinamica")
async def obtener_configuracion_dinamica():
    """Obtener configuración dinámica actual (conversión de ConfiguracionDinamicaAuditoria)"""
    try:
        conn = await get_db_connection()
        
        config_rows = await conn.fetch(
            """SELECT tabla_detectada, categoria, criticidad, tipos_evento_validos, 
                      detectada_automaticamente, fecha_deteccion
               FROM silvernostop_config_auditoria 
               WHERE activa = TRUE
               ORDER BY categoria, tabla_detectada"""
        )
        
        configuracion = []
        for row in config_rows:
            configuracion.append({
                "tabla": row['tabla_detectada'],
                "categoria": row['categoria'],
                "criticidad": row['criticidad'],
                "tipos_evento_validos": json.loads(row['tipos_evento_validos']),
                "detectada_automaticamente": row['detectada_automaticamente'],
        ...
        return {
            ...
            "total_tablas_detectadas": len(configuracion),
            "ultima_actualizacion": datetime.now().isoformat(),
            "categorias": {
                "core": [c for c in configuracion if c["categoria"] == "core"],
                "fem": [c for c in configuracion if c["categoria"] == "fem"],
                "procesamiento": [c for c in configuracion if c["categoria"] == "procesamiento"],
                "output": [c for c in configuracion if c["categoria"] == "output"],
                "sistema": [c for c in configuracion if c["categoria"] == "sistema"]
            },
            "conversion_info": "Configuración convertida desde sistema dinámico Airtable"
        }
...
async def obtener_eventos_recientes(limite: int = 50, severidad: Optional[str] = None):
    ...
    try:
        ...
        return {
            ...
            "filtro_aplicado": {"severidad": severidad, "limite": limite},
            "conversion_info": "Eventos convertidos desde SilverNonStop_AuditLog (Airtable)"
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error obteniendo eventos: {str(e)}")

# Funciones auxiliares (conversión directa de tu script)
async def validar_tabla_existe(nombre_tabla: str) -> str:
    """Validar tabla existe (conversión de validarTablaOrigen)"""
    tablas_validas = [
        "Campo_origen_despivotar",
        "Talento6X_etapas",
        "T6X_etapas_FEM", 
        "Config_FEM",
        "T6X_etapas_etiquetas",
        "Mapa_Valor",
        "Sistema_General"
    ]
    
    if not nombre_tabla or nombre_tabla not in tablas_validas:
...
def generar_hash_evento(evento: EventoAuditoriaBase) -> str:
    ...
    # Versión simplificada de hash para automatizaciones (igual que en tu script)
    import re
    clean_string = re.sub(r'[^a-zA-Z0-9]', '', data_string)
    return clean_string[:16] if len(clean_string) >= 16 else clean_string.ljust(16, '0')
...
def determinar_categoria_automatica(tabla_origen: str, tipo_evento: str) -> str:
    ...
    # Mapeo automático basado en tabla (igual que en tu script)
    mapeo_tabla = {
    ...
    # Mapeo por tipo de evento (igual que en tu script)
    mapeo_tipo = {
